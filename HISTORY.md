...
This version brings the following **NEW FEATURES**:
 - **Gameplay: Attack Damage Multiplier** server configuration controls the actual damage caused by bullets and explosions.  
   Can be set between 0.1 and 1.0. It multiplies the damage values defined in weapon config files.  
...
This version brings the following **NEW FEATURES**:
 - **Gameplay: Spectator Mode** is the initial state for all joined players now in all game modes, where they can observe the game without affecting it.  
   Spectator mode offers "free camera spectating" and "player spectating camera" mode, the latter allows switching between different players and automatically moving the camera.  
   Entering Spectator Mode is always available during gameplay by pressing 'M'.
 - **Gameplay: Team Round Game: this is a new, round-based team game mode**.   
 Similar to CS 1.6. Each round ends when all players of a team die.  
 ...
 Alternatively, the usual game time limit can kick in, in such case the team with most won rounds wins the game (unhandled case: when both teams have same amount of round wins, the team winning the 1st round earlier is the winner of the game, however since the game is not explicitly telling who is the winner in any game mode, and round win timestamps are not recorded, this is now considered as unhandled case).  
 Not deathmatch. This practially means 2 things:
   - whenever a player dies, they cannot respawn until the next round starts (kept in forced-spectating state), and
...
## Older Versions
  ...
  * [v0.4.3 Private Beta (Dec 07, 2024, ALP)](#v043-private-beta-dec-07-2024-alp)
...
Table of Contents was [generated by markdown-toc](http://ecotrust-canada.github.io/markdown-toc/).

**Meaning of ALP**: "Approofed für Lan Party": this version was actually tested on a LAN Party with at least 6 players and considered as qualified für LAN party.  
Since ALP qualification requires actual testing on a LAN party, sometimes ALP qualification is given retrospectively for a release.  
...
This version brings the following **features**:
 ...
 - **New Map:** map_construction (Agard), a fan-made map based on a real-world location!
 - **New Weapons:**
