...
proofps_dd::PRooFPSddPGE::PRooFPSddPGE(const char* gameTitle) :
    ...
    m_gui(GUI::getGuiInstance(*this, *this, m_config, m_maps, *this, m_mapPlayers, this->getSmokePool(), m_sounds)),
...
void proofps_dd::PRooFPSddPGE::onGameRunning()
{
    ...
    else
    {
        ...
        getAudio().stopSoundInstance(m_sounds.m_sndRoundWinHandle);
...
void proofps_dd::PRooFPSddPGE::mainLoopConnectedShared(PureWindow& window)
{
    ...
    updateAudioVisualsForGameModeShared(); // maybe we should do this per-tick instead of per-frame in the future
...
void proofps_dd::PRooFPSddPGE::serverRestartGame(const proofps_dd::GameRestartType_KeepPlayers& eRestartType)
{
    ...
    getAudio().stopSoundInstance(m_sounds.m_sndRoundWinHandle); // server stops it here, clients stop it when they process MsgGameSessionStateFromServer
    ...
    GameMode::getGameMode()->restartWithoutRemovingPlayers(getNetwork(), eRestartType);
