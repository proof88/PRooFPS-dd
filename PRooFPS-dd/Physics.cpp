...
void proofps_dd::Physics::serverPlayerCollisionWithWalls_bvh(const unsigned int& nPhysicsRate, XHair& xhair, proofps_dd::GameMode& gameMode, PureVector& vecCamShakeForce)
{
    ...
    for (auto& playerPair : m_mapPlayers)
    {
        ...
        if (player.getPos().getOld().getY() != player.getPos().getNew().getY())
        {
            ...
            const bool bVerticalCollisionOccured = m_maps.getBVH().findAllColliderObjects_startFromLowestLevelFittingNode(aabbPlayer, nullptr, colliders, BvhSearchDirection::DownFromRootNode);
        ...
        if (player.getWantToStandup() && !player.isSomersaulting())
        {
            if (player.getCrouchStateCurrent())
            {
                ...
                const bool bCanStandUp = (m_maps.getBVH().findOneColliderObject_startFromLowestLevelFittingNode(aabbPlayer, nullptr, BvhSearchDirection::DownFromRootNode) == nullptr);
                if (bCanStandUp)
                {
                    player.doStandupServer();
                }
        ...
        if (player.getPos().getOld().getX() != player.getPos().getNew().getX())
        {
            ...
            const PureObject3D* const pWallObj = m_maps.getBVH().findOneColliderObject_startFromLowestLevelFittingNode(aabbPlayer, nullptr, BvhSearchDirection::DownFromRootNode);
