...
protected:
    ...
    virtual void initialize() override
    {
        ...
        addSubTest(
            "test_round_games_round_ends_without_win_when_first_player_joins_a_team",
            static_cast<PFNUNITSUBTEST>(&GameModeTest::test_round_games_round_ends_without_win_when_first_player_joins_a_team));
        addSubTest(
    ...
    */
    ...
    bool test_remove_player(const proofps_dd::GameModeType& gamemode)
    {
        ...
        for (auto i = 1; i <= 2; i++)
        {
            ...
            if (bTestingAsServer && (gamemode != proofps_dd::GameModeType::TeamRoundGame))
            {
                ...
                b &= assertFalseEz(gm->wasGameWonAlreadyInPreviousTick(), gamemode, true/*server*/, "game not won previous tick");
                gm->serverTickUpdateWinningConditions(m_network);
            ...
            if (gamemode == proofps_dd::GameModeType::TeamRoundGame)
            {
                ...
                trg->getFSM().reset(); // go back to Prepare state
                ...
                b &= assertEqualsEz(
                    ...
                    trg->getFSM().getState(),
                    gamemode, true /*server*/, "FSM state 2");
                b &= assertFalseEz(gm->isGameWon(), gamemode, bTestingAsServer, "game won 4 TRG");
                ...
                if (bTestingAsServer)
                {
                    b &= assertTrueEz(gm->isGameWon(), gamemode, bTestingAsServer, "game won 6 TRG");
    ...
    bool test_restart_roundgames(const proofps_dd::GameModeType& gamemode)
    {
        ...
        for (auto i = 1; i <= 2; i++)
        {
            ...
            // cannot win yet, we are in Prepare state
            ...
            if (bTestingAsServer)
            {
                b &= assertTrueEz(gm->serverCheckAndUpdateWinningConditions(m_network), gamemode, bTestingAsServer, "winning 0");
                b &= assertTrueEz(gm->isGameWon(), gamemode, bTestingAsServer, "game won 1");
                b &= assertFalseEz(gm->wasGameWonAlreadyInPreviousTick(), gamemode, true/*server*/, "game not won previous tick 1");
                gm->serverTickUpdateWinningConditions(m_network);
                b &= assertLessEz(0, gm->getWinTime().time_since_epoch().count(), gamemode, bTestingAsServer, "win time");
    ...
    bool test_round_games_get_time_limit_in_current_state(const proofps_dd::GameModeType& gamemode)
    {
        ...
        b &= assertEqualsEz(
            static_cast<std::chrono::seconds::rep>(nRoundPrepareTimeSecs),
            trg->getFSM().getTimeLimitInCurrentStateSeconds(), gamemode, true /*server*/, "time limit 1");
    ...
    bool test_round_games_get_time_remaining_in_current_state(const proofps_dd::GameModeType& gamemode)
    {
            ...
            (trg->getFSM().getState() != proofps_dd::TeamRoundGameMode::RoundStateFSM::RoundState::Play))
        {
            ...
            b &= assertLequalsEz(
                ...
                gamemode, true /* testing as server */,
                "millisecs 1");
        }
        ...
        {
            b &= assertLequalsEz(nRoundPrepareTimeSecs, *itSetRemainingSecs, gamemode, true /* testing as server */, "remaining 1 set elem");
            ++itSetRemainingSecs;
        }
        ...
        /* -------------- Test 2: Play -> WaitForReset --------------------------------------- */

        setRemainingSecs = { 0, 1, 2, 3, 4, 5 };
        iSleep = 0;
        while ((iSleep++ < 10) &&
            !trg->serverCheckAndUpdateWinningConditions(m_network) /* this drives RoundStateFSM */ &&
            (trg->getFSM().getState() != proofps_dd::TeamRoundGameMode::RoundStateFSM::RoundState::WaitForReset))
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(400));
        ...
        // we erased valid elements from the set, so only remaining elements that were too big to be remaining time
        b &= assertFalseEz(setRemainingSecs.empty(), gamemode, true /* testing as server */, "no remaining 2");

        itSetRemainingSecs = setRemainingSecs.begin();
        while (b && (itSetRemainingSecs != setRemainingSecs.end()))
        {
            b &= assertLequalsEz(nRoundTimeLimitSecs, *itSetRemainingSecs, gamemode, true /* testing as server */, "remaining 2 set elem");
            ++itSetRemainingSecs;
        }
        ...
        b &= assertGequals(5u, trg->getFSM().getTimeRemainingInCurrentStateSeconds(), "remaining secs in WaitForReset state 1") &
            assertNotEquals(static_cast<std::chrono::seconds::rep>(0), trg->getFSM().getTimeRemainingInCurrentStateSeconds(), "remaining secs in WaitForReset state 2");
        b &= assertGequals(5 * 1000, trg->getFSM().getTimeRemainingInCurrentStateMilliseconds(), "remaining millisecs in WaitForReset state 1") &
            assertNotEquals(static_cast<std::chrono::seconds::rep>(0), trg->getFSM().getTimeRemainingInCurrentStateMilliseconds(), "remaining millisecs in WaitForReset state 2");
        
        /* -------------- Test 3: WaitForReset -> Prepare --------------------------------------- */
        ...
        /* check: we exited the loop because time elapsed the FSM has transitioned to next state */
        b &= assertTrueEz(setRemainingSecs.empty(), gamemode, true /* testing as server */, "no remaining 3");
        b &= assertEqualsEz(
            proofps_dd::TeamRoundGameMode::RoundStateFSM::RoundState::Prepare,
            trg->getFSM().getState(),
    ...
    bool test_round_games_transition_to_play_state_after_n_seconds(const proofps_dd::GameModeType& gamemode)
    {
        ...
        std::set<std::chrono::seconds::rep> setRemainingSecs = { 0, 1, 2, 3 };
        int iSleep = 0;
        while ((iSleep++ < 10) &&
            !trg->serverCheckAndUpdateWinningConditions(m_network) /* this drives RoundStateFSM */ &&
            (trg->getFSM().getState() != proofps_dd::TeamRoundGameMode::RoundStateFSM::RoundState::Play))
        ...
        /* check: we exited the loop because time elapsed the FSM has transitioned to next state */
        b &= assertFalseEz(setRemainingSecs.empty(), gamemode, true /* testing as server */, "remaining");

        auto itSetRemainingSecs = setRemainingSecs.begin();
        while (b && (itSetRemainingSecs != setRemainingSecs.end()))
        {
            b &= assertLessEz(nRoundPrepareTimeSecs, *itSetRemainingSecs, gamemode, true /* testing as server */, "remaining set elem");
            ++itSetRemainingSecs;
        }
    ...
    bool test_round_games_round_ends_without_win_when_first_player_joins_a_team(const proofps_dd::GameModeType& gamemode)
    {
        ...
        if (b)
        {
            // player is dead, forced-spectating initially
            //player1.setHealth(100);
            //player1.updateOldValues();
            player1.setName("Adam");
            player1.getFrags() = 0;
        ...
        /* drive FSM: round shall end due to the only player who joined is dead (initial game start scenario) */
        b &= assertFalseEz(
            trg->serverCheckAndUpdateWinningConditions(m_network), gamemode, true /* testing as server */, "serverCheck 1");
        b &= assertFalseEz(gm->isGameWon(), gamemode, true /* testing as server */, "game won 1");
        b &= assertEqualsEz(
    ...
    bool test_round_games_round_ends_without_win_when_first_player_joins_a_team()
    {
        ...
        bool b = true;
        b &= assertTrue(
            test_round_games_round_ends_without_win_when_first_player_joins_a_team(gamemode),
            proofps_dd::GameMode::getGameModeTypeName(gamemode));
    ...
    bool test_round_games_round_ends_without_win_when_second_player_joins_the_other_team(const proofps_dd::GameModeType& gamemode)
    {
        ...
        if (b)
        {
            ...
            player1.getTeamId() = 1; // required non-zero teamId for TDM test, otherwise TDM does not count frags
            player1.isInSpectatorMode() = false; // otherwise player won't be taken into account for their assigned team, gamemode win state, etc.
            player1.setForcedSpectating(false);
        }
        ...
        // 2nd player joins team2 (dead by default)
        insertRes = m_mapPlayers.insert(
            {
                2,
                proofps_dd::Player(
        ...
        if (b)
        {
            // player2 in team2 is dead since just joined team2
            //player2.setHealth(100);
            //player2.updateOldValues();
            player2.setName("Banana");
            player2.getFrags() = 0;
            player2.getDeaths() = 0;
            player2.getTeamId() = 2; // required non-zero teamId for TDM test, otherwise TDM does not count frags
            player2.isInSpectatorMode() = false; // otherwise player won't be taken into account for their assigned team, gamemode win state, etc.
