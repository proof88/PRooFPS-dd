...
private:
    ...
    bool test_gen_unique_user_name()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            const auto insertRes = mapPlayers.insert(
                {
                    connHandle,
                    proofps_dd::Player(m_audio, m_cfgProfiles, m_bullets, m_itemPickupEvents, m_inventoryChangeEvents, m_ammoChangeEvents, *m_engine, m_network, connHandle, std::string("192.168.1.") + std::to_string(i))
                });
            b &= insertRes.second;
            if (!b)
            {
    ...
    bool test_initial_values()
    {
        ...
        proofps_dd::Player player(m_audio, m_cfgProfiles, m_bullets, m_itemPickupEvents, m_inventoryChangeEvents, m_ammoChangeEvents, *m_engine, m_network, connHandleExpected, sIpAddr);
        ...
        return (assertEquals(connHandleExpected, player.getServerSideConnectionHandle(), "connhandle") &
            ...
            assertEquals(0, playerConst.getHealth(), "health") &
            ...
            assertFalse(player.getDescentInput().isDirty(), "old descent") &
            assertFalse(player.getDescentInput(), "descent") &
    ...
    {
        const pge_network::PgeNetworkConnectionHandle connHandleExpected = static_cast<pge_network::PgeNetworkConnectionHandle>(12345);
        proofps_dd::Player player(m_audio, m_cfgProfiles, m_bullets, m_itemPickupEvents, m_inventoryChangeEvents, m_ammoChangeEvents, *m_engine, m_network, connHandleExpected, "192.168.1.12");
    ...
    bool test_dirtiness_one_by_one()
    {
        ...
        b &= assertTrue(player.isDirty(), "dirty I 1");
        b &= assertFalse(player.isNetDirty(), "net dirty I 1");
        player.updateOldValues();
        b &= assertFalse(player.isDirty(), "dirty I 2");
        ...
        player.getDescentInput().set(true);
        b &= assertTrue(player.isDirty(), "dirty L 1");
        b &= assertFalse(player.isNetDirty(), "net dirty L 1");
        player.updateOldValues();
        b &= assertFalse(player.isDirty(), "dirty L 2");
        b &= assertTrue(player.isNetDirty(), "net dirty L 2");
        player.clearNetDirty();
        b &= assertFalse(player.isNetDirty(), "net dirty L 3");
    ...
    bool test_set_health_and_update_old_health()
    {
        ...
        player.setHealth(100);
        b &= (assertEquals(100, playerConst.getHealth(), "health 1") &
        ...
        player.updateOldValues();
        b &= assertTrue(playerConst.isNetDirty(), "is net dirty");
        b &= (assertEquals(5, playerConst.getHealth().getOld(), "health 4") &
            assertFalse(playerConst.getHealth().isDirty(), "dirty 7") &
            assertFalse(playerConst.isDirty(), "dirty 8")) != 0;
